...
public class ListaDuplamenteEncadeada<T> {
	...
	public void trocaPosicao(int posicao) {
		if (this.quantidadeElementos == 0) {
			...
			throw new ArrayIndexOutOfBoundsException("A lista possue somente um elemento!");
		} else if (posicao == (this.quantidadeElementos - 1)) {
			throw new ArrayIndexOutOfBoundsException("A posição informada é a última da lista, por tanto nada será alterado.\nPor favor, escolha outra posição!");
		} else if (posicao >= this.quantidadeElementos || posicao < 0) {
			throw new ArrayIndexOutOfBoundsException("Posição invalida!");
		} else {
			T elemento = this.recupera(posicao);
